...
export default function useFormValidation() {
  ...
  const validateForm = ({ email, subject, content, privacyChecked }: FormValidationProps): boolean => {
    let isValid = true;
    ...
    if (emailRef.current) {
      if (!email) {
        ...
        emailRef.current.setCustomValidity('올바른 이메일 주소를 입력해주세요.');
        isValid = false;
      } else {
    ...
    if (privacyRef.current) {
      if (!privacyChecked) {
        privacyRef.current.setCustomValidity('개인정보 수집 및 이용에 동의해주세요.');
        isValid = false;
      } else {
  ...
  const validateField = (
    field: 'content' | 'email' | 'privacy' | 'subject',
  ...
  const clearValidationMessage = (field: 'content' | 'email' | 'privacy' | 'subject') => {
    switch (field) {
      case 'email':
